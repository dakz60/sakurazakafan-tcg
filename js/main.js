...
function removeFromDeck(id){ const index=deck.indexOf(id); if(index!==-1) deck.splice(index,1); if(territoryCardId===id) territoryCardId=null; updateDeckImages(); updateDeckStatus(); }
...
function toggleBackground(){ const deck = document.getElementById("deckContainer"); if(deck.classList.contains("white-mode")){ deck.classList.replace("white-mode","mat-mode"); }else{ deck.classList.replace("mat-mode","white-mode"); } }
...
function downloadRecipeImage(){
  ...
  Promise.all(loadImages).then(images=>{
    images.forEach((img,i)=>{
      ...
      const x = padding + col * (cardWidth + margin);
      const y = padding + 40 + row * (cardHeight + margin);
