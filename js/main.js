...
function addToDeck(id) {
  ...
  const sameSpecCount = deck.filter(d => {
    ...
    if (!c) return false;
    return (c.name === card.name && c.type === card.type && c.cost === card.cost &&
      c.power === card.power && c.hit === card.hit && c.color === card.color &&
      JSON.stringify(c.subType||[]) === JSON.stringify(card.subType||[]) &&
      c.generation === card.generation);
  }).length;
...
function checkDeckRules(){
  ...
  const terr = territoryCardId?1:0;
...
function generateDeckCode(){
  ...
  const territoryIdStr = territoryCardId||"0";
  document.getElementById("deckCodeBox").value = territoryIdStr + "|" + idList;
...
function loadDeckFromCode(){
  ...
  try{
    ...
    const territoryIdStr = parts[0];
    const deckIds = parts[1]?parts[1].split(","):[];
    territoryCardId = territoryIdStr!=="0"?territoryIdStr:null;
...
function copyDeckCode(){
  ...
  navigator.clipboard.writeText(box.value)
    .then(()=>{ document.getElementById("deckCodeResult").textContent="コピーしました！"; })
...
function toggleBackground(){
  ...
  if(deckEl.classList.contains("white-mode")) deckEl.classList.replace("white-mode","mat-mode");
  else deckEl.classList.replace("mat-mode","white-mode");
}
...
function tweetFavoriteFive(){
  if(favoriteFive.length===0){ alert("カードを選んでください"); return; }
