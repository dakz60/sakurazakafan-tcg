...
function showCollection(){
  ...
  container.innerHTML = cards
    ...
    .map(card => {
      ...
      // 背景・文字色の決定
      let bgStyle = '#F3F3F3';   // デフォルト背景
      let nameColor = '#F6A5B8'; // デフォルト文字
      return `
...
function showBoard(){
  ...
  // 🔥 譲・求 分離検索
  if(giveSearch) list = list.filter(p => p.give.includes(giveSearch));
  if(wantSearch) list = list.filter(p => p.want.includes(wantSearch));
